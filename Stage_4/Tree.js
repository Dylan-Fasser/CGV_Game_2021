...
class Tree{
    ...
    constructor(type = 'light'){
        ...
        if(type === 'light') {
            ...
            this._buildDarkTreeTop('./textures/red_tree.jpg');
    ...
    _generateCone(texture = './textures/tree.jpg'){
        ...
        const geometry = new THREE.ConeGeometry( 0.5, 1.2, 32 );
        const material = new THREE.MeshBasicMaterial( {color: 0x006400, map:TextureLoader} );
        return new THREE.Mesh(geometry, material);

    }
    _generateDarkCone(){
        let TextureLoader = new THREE.TextureLoader().load("textures/dark_leaf.PNG");
        TextureLoader.wrapS = THREE.RepeatWrapping;
        TextureLoader.wrapT = THREE.RepeatWrapping;
        TextureLoader.repeat.set(5,5);

        const geometry = new THREE.ConeGeometry( 0.5, 1.2, 5 );
